...
import { useDeleteCatalogItem } from "@/domain/catalog/useCases/use-delete-catalog-item";
...
function CatalogItemDetails({ dto }: { dto: CatalogItemDTO }): React.ReactElement {
  const typeLabel = dto.type === "PRODUCT" ? "Produto" : "Serviço";
  ...
  return (
    <dl className="grid gap-[0.75rem] rounded-[0.75rem] border border-zinc-200 p-[1rem] dark:border-zinc-800">
      ...
      <DetailRow label="Descrição" value={formatValue(dto.description)} />
      <DetailRow label="Preço de venda" value={formatMoney(dto.price)} />
      {dto.type === "PRODUCT" ? (
        <>
          <DetailRow label="Custo" value={formatMoney(dto.costPrice)} />
          <DetailRow label="Estoque" value={formatStock(dto.stockQuantity)} />
          <DetailRow label="Unidade" value={formatValue(dto.unit)} />
          <DetailRow label="Código de barras" value={formatValue(dto.barcode)} />
        </>
      ) : (
        <DetailRow
          ...
          value={
            dto.estimatedDurationMinutes !== null
              ? `${dto.estimatedDurationMinutes} min`
              : "—"
          }
        />
      )}
      ...
      <DetailRow
        label="Criado em"
        value={new Date(dto.createdAt).toLocaleString("pt-PT", {
          dateStyle: "short",
          timeStyle: "short",
        })}
      />
      <DetailRow
        label="Atualizado em"
        value={new Date(dto.updatedAt).toLocaleString("pt-PT", {
          dateStyle: "short",
          timeStyle: "short",
        })}
      />
...
export function CatalogDetailPage({ id }: CatalogDetailPageProps): React.ReactElement {
  ...
  function handleDeactivate(): void {
    ...
    const ok = window.confirm(
      "Desativar este item? Poderá voltar a incluir inativos na lista.",
    );
    if (!ok) return;
